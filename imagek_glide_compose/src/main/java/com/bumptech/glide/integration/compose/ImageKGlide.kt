...
import com.mozhimen.basick.utilk.android.content.gainDrawable
    ...
    if (loadingComposable != null || failureComposable != null) {
        GlideSubcomposition(model, modifier, { requestBuilder }) {
            if (state == RequestState.Loading && loadingComposable != null) {
                onLoading?.invoke(state as RequestState.Loading)
                loadingComposable()
            } else if (state == RequestState.Failure && failureComposable != null) {
                onSuccess?.invoke(state as RequestState.Success)
                failureComposable()
            } else {
                onError?.invoke(state as RequestState.Failure)
                Image(
                    painter,
                    contentDescription,
                    modifier,
 ...
 *
 * [ImageKGlide] is significantly more efficient and easier to use than this method. GlideImage
 ...
 * As with [ImageKGlide] try to ensure that you either set a reasonable [RequestBuilder.override]
    ...
    val painter =
        when (loading) {
            ...
            is Placeholder.OfResourceId ->
                LocalContext.current.gainDrawable(loading.resourceId).toPainter()
...
/**
 * Used to specify a [Drawable] to use in conjunction with [ImageKGlide]'s `loading` or `failure`
 * parameters.
 *
...
/**
 * Used to specify a resource id to use in conjunction with [ImageKGlide]'s `loading` or `failure`
 * parameters.
 *
...
/**
 * Used to specify a [Painter] to use in conjunction with [ImageKGlide]'s `loading` or `failure`
 * parameters.
 */
@ExperimentalGlideComposeApi
...
/**
 * Used to specify a [Composable] function to use in conjunction with [ImageKGlide]'s `loading` or
 * `failure` parameter.
 *
 * Providing a nested [ImageKGlide] is not recommended. Use [RequestBuilder.thumbnail] or
 ...
 * one provided to [ImageKGlide]. Doing so allows Glide to infer the requested size if one is not
